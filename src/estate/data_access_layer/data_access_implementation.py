...
class MySQLDataAccessLayer(DataAccessInterface):
    ...
    def read_properties_without_filters(self):
        ...
        AND sh.update_date = (SELECT MAX(update_date)
        FROM status_history WHERE property_id = p.id );
        '''
    ...
    def read_properties_with_filters(self, *args: tuple):
        ...
        WHERE ((s.name = 'pre_venta' OR s.name = 'en_venta'

        OR s.name = 'vendido')

        AND sh.update_date = (SELECT MAX(update_date) FROM
        status_history WHERE property_id = p.id ))
